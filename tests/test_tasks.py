...
def test_filter_and_pagination(client: TestClient, auth_headers: dict[str, str]):
    for i in range(5):
        client.post(
            "/api/v1/tasks",
            headers=auth_headers,
...
def test_user_cannot_access_another_users_task(client: TestClient):
    ...
    task = client.post("/api/v1/tasks", headers=alice, json={"title": "Alice secret"})
    task_id = task.json()["id"]

    assert client.get(f"/api/v1/tasks/{task_id}", headers=bob).status_code == 404
    assert client.patch(f"/api/v1/tasks/{task_id}", headers=bob, json={"title": "hack"}).status_code == 404
